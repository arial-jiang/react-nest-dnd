...
export const onCalcPos = (atIndex, index) => {
...
export const onIsDrop = (dropItem, atIndex, index) => {
  ...
  const isSameLayer = depth === draggedDepth;
  const isChildLayer =
    draggedDepth - depth === 1 && index[index?.length - 1] === 0;
  const isToChildLayer = depth - draggedDepth === 1 && group !== atGroup;
